...
- Format: `MAJOR.MINOR.PATCH+BUILD` (e.g., `1.19.0+1`)
...
3. **Update Cargo.lock**:
   ```bash
   cd src-tauri && cargo update -p numbat
   ```
...
5. **Update Android properties** in `src-tauri/gen/android/app/tauri.properties`:
   ```properties
   tauri.android.versionName=1.19.0+1
   tauri.android.versionCode=1190001
   ```
...
6. **Update iOS version** in both:
   ...
   Set `CFBundleShortVersionString` and `CFBundleVersion` to `1.19.0`
...
8. **Commit and tag**:
   ```bash
   git commit -am "Bump version to 1.19.0+1"
   git tag v1.19.0.1
   git push && git push --tags
   ```

CI will build and attach the APK and AAB files to the GitHub release.
